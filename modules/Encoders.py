...
class Pseq2Sites(nn.Module):
    def __init__(self, config):
        ...
        self.PocketConv = PocketConvLayer(self.config, 1024, 256, 1, 1)
        self.PocketAttn = PocketATTLayer(self.config)
    ...
    def forward(self, prots, total_prots_data, attention_mask, position_ids, token_type_ids):
        ...
        # input features
        feats = prots + position_embeddings + token_type_embeddings
        ...
        feats = self.PocketConv(query_embeddings, feats, prots_extended_attention_mask)
        feats, att_probs = self.PocketAttn(query_embeddings, feats, prots_extended_attention_mask)
        
        outputs = self.fc2(torch.cat([feats, prot_feats], dim = 2)).squeeze(-1)
        
        return outputs, feats, att_probs
...
class PocketConvLayer(nn.Module):
    def __init__(self, config, dim_in, dim_out, padding = 1, stride = 1):
        ...
        for idx, dilation_rate in enumerate(dilation_tuple):
            self.first_.append(PocketEncoderLayer(
                self.config,
                dim_in = dim_in_tuple[idx],
                dim_out = dim_out_tuple[idx], 
                kernel_size = 3,
        ...
        for idx, dilation_rate in enumerate(dilation_tuple):
            self.second_.append(PocketEncoderLayer(
                self.config,
                dim_in = dim_in_tuple[idx],
                dim_out = dim_out_tuple[idx], 
                kernel_size = 5,
        ...
        for idx, dilation_rate in enumerate(dilation_tuple):
            self.third_.append(PocketEncoderLayer(
                self.config,
                dim_in = dim_in_tuple[idx],
                dim_out = dim_out_tuple[idx], 
                kernel_size = 7,
    ...
    def forward(self, query_embeddings, aa_embeddings, attention_mask):    
        first_aa_embeddings = aa_embeddings.transpose(1,2)
        second_aa_embeddings = aa_embeddings.transpose(1,2)
        third_aa_embeddings = aa_embeddings.transpose(1,2)
